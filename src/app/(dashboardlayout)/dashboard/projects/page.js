...
const usePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const startData = form.startData.value;
    const info = {
      title,
      description,
      team,deadline,startData
    }
    console.log(info);

//projects
    try {
      const res = await axios.post('http://localhost:5000/projects', info)
      if (res.data?.success) {
        e.target.reset()
        Swal.fire({
          position: "top-end",
                ...
                <form onSubmit={handleSubmit}>
                  <div className="grid gap-6 mb-6 md:grid-cols-2">
                    ...
                    <div>
                      ...
                      <input type="date" name="startData" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " placeholder="Project Deadline" required />
                    </div>
                    <div>
                      <label className="block mb-2 text-sm font-medium text-gray-900 ">Deadline</label>
                      <input type="date" name="Deadline" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " placeholder="Project Deadline" required />
                    </div>
                    <div>
